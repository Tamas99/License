    ...
    for dirname, _, filenames in os.walk(dataFolder):
        ...
        for filename in filenames:
            ...
            for i in range(actions.last_valid_index() + 1):
                ...
                axs[0, 0].set(xlabel='idő', ylabel='érték')
                axs[0, 0].plot(dx, '-o')
                axs[0, 1].set_title('dy')
                axs[0, 1].set(xlabel='idő', ylabel='érték')
                axs[0, 1].plot(dy, '-o')
                axs[1, 0].set_title('dt')
                axs[1, 0].set(xlabel='idő', ylabel='érték')
                axs[1, 0].plot(dt, '-o')
                axs[1, 1].set_title('röppálya')
                axs[1, 1].set(xlabel='x', ylabel='y')
                axs[1, 1].plot(x, y, '-o')
                axs[1, 1].plot(x[0:1], y[0:1], 'ro', label='Start')
                axs[1, 1].legend()
                fig.tight_layout()
                axs.invert_yaxis()
                try:
...
def subplotCurrentActions(currentData, currentActions, userid, maxes, mins, minute):
    for i in range(currentActions.last_valid_index() + 1):
        ...
        axs[1, 1].set(xlabel='X', ylabel='Y')
        axs[1, 1].plot(x, y, '.')
        axs[1, 1].plot(x[0:1], y[0:1], 'r.', label='Starting')
        axs[1, 1].legend()
        fig.tight_layout()
        axs.invert_yaxis()
